...
현재 active v1 목표는 한글 음가 transcript 안정화다. 개발 literal 보존과 복원은 v2 이후 token recovery 또는 command normalization 평가 축으로 분리한다.

주요 관심사:

- 한국어 명령 인식.
- 한영 혼합 발화의 한글 음가 인식.
- 파일명과 경로를 읽은 한글 음가 인식.
- CLI option을 읽은 한글 음가 인식.
- 모듈명, 함수명, 코드 식별자를 읽은 한글 음가 인식.
- hallucination 방어.
- insertion-safe 여부.
...
- `text_comparison.expected_text`: v1에서 기대하는 한글 음가 STT transcript.
...
v1 pass 기준은 `phonetic_transcript_match`, `normalized_char_error_rate`,
`text_similarity`다. 파일 경로, CLI option, 코드 식별자, 한영 혼합 발화의
literal 보존 지표는 v2 literal 복원 판단을 위한 참고 지표다.
...
- `phonetic_transcript_mismatch`: v1 한글 음가 expected transcript와 recovered transcript의 normalized match 실패.
- `latin_token_loss`: expected의 Latin-script token 보존 실패. v1 통과 기준이 아니라 v2 literal 복원 참고 지표다.
- `file_path_loss`: 파일명 또는 경로 token 보존 실패. v1 통과 기준이 아니라 v2 literal 복원 참고 지표다.
- `cli_option_loss`: `--option` 형태 CLI option 보존 실패. v1 통과 기준이 아니라 v2 literal 복원 참고 지표다.
- `code_identifier_loss`: 함수명, 모듈명, snake_case, dotted identifier 보존 실패. v1 통과 기준이 아니라 v2 literal 복원 참고 지표다.
...
- `expected transcript`: v1에서 기대하는 한글 음가 STT transcript.
