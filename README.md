...
- 로컬 STT 모델: Whisper/Hugging Face/faster-whisper/whisperx 캐시 또는 설치본 없음.
...
3. 한국어와 한영 혼합 문장의 정확도 실험을 기록한다.
4. 변환 결과를 클립보드에 복사한다.
5. 누르고 말하기 방식의 입력 UX를 만든다.
6. 녹음, STT, 클립보드 복사를 하나의 명령으로 묶는다.
7. 실제 Codex CLI 입력 흐름에서 반복 사용성을 검증한다.
8. 정확도, 오인식 사례, 처리 시간을 기준으로 모델과 UX를 조정한다.
