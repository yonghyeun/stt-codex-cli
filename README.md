...
실행 후 사용자는 `Ctrl+T`를 한 번 눌러 녹음을 시작하고, 말을 끝낸 뒤 `Ctrl+T`를 다시 한 번 눌러 녹음을 종료한다. 발화 중간에 버려야 하면 `Esc`로 현재 녹음을 취소한다. wrapper는 정상 종료된 녹음의 STT raw transcript만 Codex CLI 입력창에 삽입한다. Enter는 사용자가 직접 누른다.
