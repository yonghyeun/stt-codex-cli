...
- 기본값에서는 사용자가 입력 내용을 확인한 뒤 직접 전송한다.
...
사용자 실행
  ...
  -> 기본 review mode에서는 사용자가 확인 후 Enter
  -> opt-in auto mode에서는 wrapper가 Enter 전송
```

녹음 중 `Esc`를 누르면 현재 녹음을 취소하고 audio를 폐기한다. 취소된 녹음은 STT로 보내지 않고 child PTY에도 삽입하지 않는다.
...
- 기본값에서는 Codex CLI 자동 전송을 하지 않는다. Enter는 사용자가 직접 입력한다.
- `--submit-mode auto`는 opt-in 실험 기능으로만 Codex CLI 자동 전송을 수행한다.
...
| submit-time | `--submit-mode` | 삽입된 text를 사용자가 검토할지, text가 있을 때 즉시 Enter를 보낼지 결정한다. 기본값은 `review`다. |
...
실행 후 사용자는 `Ctrl+T`를 한 번 눌러 녹음을 시작하고, 말을 끝낸 뒤 `Ctrl+T`를 다시 한 번 눌러 녹음을 종료한다. 발화 중간에 버려야 하면 `Esc`로 현재 녹음을 취소한다. wrapper는 정상 종료된 녹음의 STT raw transcript만 Codex CLI 입력창에 삽입한다. 기본 `--submit-mode review`에서는 Enter를 사용자가 직접 누른다.

자동 전송 실험이 필요할 때만 opt-in으로 켠다.

```bash
scripts/stt_codex.py --submit-mode auto
STT_SUBMIT_MODE=auto scripts/stt_codex.py
```

`--submit-mode auto`에서도 empty transcript, 짧은 녹음, STT error, Esc cancel 경로는 Enter를 전송하지 않는다. Child PTY 입력창에 기존 text가 있으면 기존 text와 transcript가 함께 제출될 수 있으므로 반복 사용 전 입력창 상태를 확인한다.
...
- 녹음 중 `Esc`는 child PTY로 전달되지 않고 parent wrapper가 cancel로 소비한다. 녹음 중이 아니면 기존처럼 child PTY로 전달된다.
...
- 사용자가 `Ctrl+T`로 녹음을 시작하면 `STT recording 중 00:05 / 01:00 | Ctrl+T stop | Esc cancel`처럼 현재/최대 녹음 시간과 cancel affordance가 표시된다.
- 녹음 중 `Esc`를 누르면 `STT canceled | Ctrl+T retry`가 표시되고 STT 변환과 child PTY 삽입을 건너뛴다.
- STT raw transcript가 Codex CLI 입력창에 삽입된다.
- 기본 `--submit-mode review`에서는 Enter가 자동 전송되지 않는다.
- `--submit-mode auto`에서는 text가 있는 transcript가 삽입된 뒤 Enter가 전송된다.
- review mode에서는 사용자가 삽입된 문장을 수정하거나 그대로 Enter로 전송할 수 있다.
